...
namespace
{
	const char* const WINDOW_TITLE = "6-3 Lighting a Pyramid"; // Macro for window title
...
int main(int argc, char* argv[])
{
	...
	// Load texture
	const char* texFilename = "sand.jpg";
...
void URender()
{
	...
	//set ambient color
	glUniform3f(ambColLoc, 0.58f, 0.58f, 0.58f);
	glUniform3f(light1ColLoc, 0.0f, 0.44f, 0.21f);
	glUniform3f(light1PosLoc, -2.0f, 5.0f, 2.0f);
	glUniform3f(light2ColLoc, 0.61f, 0.04f, 0.05f);
	...
	rotation = glm::rotate(0.0f, glm::vec3(1.0, 1.0f, 1.0f));
	// 3. Place object at the origin
	translation = glm::translate(glm::vec3(0.0f, 2.1f, 0.0f));
	// Model matrix: transformations are applied right-to-left order
	model = translation * rotation * scale;
